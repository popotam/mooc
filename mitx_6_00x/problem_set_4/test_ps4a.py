...
# (open up in IDLE, then run the file as normal)
...
def test_getWordScore():
    ...
    for (word, n) in words.keys():
        ...
        if score != words[(word, n)]:
            ...
            print "points but got '" + str(score) + "' for word '" + \
                    word + "', n=" + str(n)
            failure = True
    if not failure:
...
def test_isValidWord(wordList):
    ...
    if not isValidWord(word, handCopy, wordList):
        ...
        if handCopy != handOrig:
            ...
            print "for a second time - be sure you're not modifying hand."
            print "\tAt this point, hand ought to be", handOrig, "but it is",
            print handCopy

        else:
            print "\tTesting word", word,
            print "for a second time - have you modified wordList?"
    ...
    if  isValidWord(word, hand, wordList):
        ...
        print "\tExpected False, but got True for word: '" + word \
                + "' and hand:", hand
        print ("\t(If this is the only failure, make sure isValidWord() "
