...
def buildCoder(shift):
    ...
    for letters in (string.ascii_lowercase, string.ascii_uppercase):
        coder.update((letter, letters[index % 26])
                     for index, letter in enumerate(letters, shift))
    return coder
...
def findBestShift(wordList, text):
    ...
    """
    return max(
        (sum((word.strip(string.punctuation) in wordList)
             for word in applyShift(text, shift).split()),
         shift) for shift in reversed(xrange(26))
    )[1]
