...
class SimpleVirus(object):
    ...
    def __init__(self, maxBirthProb, clearProb):
        ...
        """
        self.maxBrithProb = maxBirthProb
...
class Patient(object):
    ...
    def update(self):
        ...
        """
        # clear
        self.viruses = [virus for virus in self.getViruses()
                        if not virus.doesClear()]
        # density
        density = float(self.getTotalPop()) / self.getMaxPop()
        # reproduce
        offspring = []
        for virus in self.getViruses():
            try:
                offspring.append(virus.reproduce(density))
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    # calculate means
    population = [cumul / float(numTrials) for cumul in population]
...
class ResistantVirus(SimpleVirus):
    ...
    def __init__(self, maxBirthProb, clearProb, resistances, mutProb):
        """Initialize a ResistantVirus instance, saves all parameters
        ...
        to each drug. e.g. {'guttagonol':False, 'srinol':False}, means that
        this virus particle is resistant to neither guttagonol nor srinol.
    ...
    def isResistantTo(self, drug):
        ...
        This method is called by getResistPop() in TreatedPatient
        to determine how many virus particles have resistance to a drug.
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        A virus particle will only reproduce if it is resistant to ALL
        the drugs in the activeDrugs list. For example, if there are
        2 drugs in the activeDrugs list, and the virus particle
        is resistant to 1 or no drugs, then it will NOT reproduce.
        ...
        virus = ResistantVirus(self.getMaxBirthProb(), self.getClearProb(),
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        """
        return len([virus for virus in self.viruses
    ...
    def update(self):
        ...
        for virus in self.getViruses():
            try:
                kid = virus.reproduce(density, self.getPrescriptions())
