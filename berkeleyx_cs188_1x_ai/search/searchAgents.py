...
CornersState = collections.namedtuple("CornersState", "position corners")
...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        return CornersState(self.startingPosition, (False,) * 4)

    def isGoalState(self, state):
        "Returns whether this search state is a goal state of the problem"
        return all(state.corners)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH,
            ...
            new_pos, hits_wall = self.try_action(state.position, action)
            if hits_wall:
                continue
            corner_state = list(state.corners)
            if new_pos in self.corners:
                corner_state[self.corners.index(new_pos)] = True
            successor = (
                CornersState(new_pos, tuple(corner_state)),
                action,
                1,
            )
    ...
    def try_action(self, position, action):
        x, y = position
        dx, dy = Actions.directionToVector(action)
        nextx, nexty = int(x + dx), int(y + dy)
        hits_wall = self.walls[nextx][nexty]
        return (nextx, nexty), hits_wall
    ...
    def getCostOfActions(self, actions):
        ...
        """
        print "getCostOfActions", actions
        if actions == None:
            return 999999
...
def cornersHeuristic(state, problem):
    ...
    x, y = state.position
    distances = [abs(x - cx) + abs(y - cy)
                 for corner, (cx, cy) in enumerate(corners)
                 if not state.corners[corner]]
...
def foodHeuristic(state, problem):
    ...
    print position, foodGrid.asList(), distances
    return max(distances)
    """
