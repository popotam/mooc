...
public class WeightedTFIDFItemScorer extends AbstractItemScorer {
    ...
    private SparseVector makeUserVector(long user) {
        ...
        for (Rating r: userRatings) {
            ...
            if (p != null) {
                sumRating += p.getValue();
                numRatings++;
            }
        }
        //System.out.format("User %s mean rating %s / %s = %s\n",
        //        user, sumRating, numRatings, sumRating / numRatings);
        ...
        for (Rating r: userRatings) {
            ...
            if (p != null) {
                ...
                for (VectorEntry e: miv.fast()) {
                    // TODO Update this document frequency entry to be a log-IDF value
                    miv.set(e.getKey(), (p.getValue() - sumRating / numRatings) * e.getValue());
                }
                profile.add(miv.freeze());
            }
        }
